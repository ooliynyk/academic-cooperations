...
public class ProxiedDocumentParser implements DocumentParser {
	...
	@Value("${connection.proxy-server.host:127.0.0.1}")
	private String proxyServerHost;
	
	@Value("${connection.proxy-server.port:8118}")
	private int proxyServerPort;
	
	@Value("${connection.always-proxied:true}")
	private boolean alwaysProxied;

	public Document parseDocument(String url) throws DocumentParsingException {
		Document doc = null;
		for (int attempt = 1; attempt <= 2; attempt++) {
			try {
				Proxy proxy = (alwaysProxied || proxied.get()) ? new Proxy(Proxy.Type.HTTP,
				...
				if (!alwaysProxied && proxied.compareAndSet(false, true)) {
